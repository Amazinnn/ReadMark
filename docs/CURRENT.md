...
- Runner shared semantic preparation is implemented and independently accepted: Term materialization and nonliteral audit, explicit deterministic anomaly input, stratified Source Disposition review/adjudication, strict request caps, Generic Filter discard boundaries, and Attribute proposal/critic/repair.
- Atomic Runner candidate publication and the explicit-cluster three-Run executor are implemented and independently accepted. Deterministic Evidence Cluster construction and final revision-assembly integration remain pending; plugin Source Snapshot/import and Semantic Report activation are not complete.
- Versioned checkpoints now exist for documentation, protocol/scoring, Prompt runtime, and shared semantic preparation. The plugin documentation branch is pushed; the Runner feature branch has local commits but no configured remote.
...
Resolve the retained Evidence Cluster formula gate, integrate shared preparation to three Runs and deterministic Revision publication, then implement plugin Source Snapshot/import and the Semantic Report before any 2.5D work.
