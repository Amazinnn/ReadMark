...
The final independent-review dispatch gate was unavailable because the local constrained-reviewer calibration state is disabled; no reviewer `ACCEPT` was inferred. Main-thread review instead found one concrete recovery defect: a stale interrupted Bundle was correctly rejected but the empty-report UI offered only repeated resume. The accepted UI now presents both exact resume and explicit new-task actions whenever a task reference exists. Main wiring remains 4/4, orchestration 4/4, replacement E2E 1/1, the fresh Plugin aggregate suite passes, and no-emit TypeScript passes after this correction.
