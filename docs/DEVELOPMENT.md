...
Runner commit `91e4658` accepts the Q503 implementation boundary. Attribute preparation now accepts the canonical Snapshot plus finalized `TermBinding` records and invokes the projection internally; no production caller can pass free-form distribution buckets. Attribute Proposal, Critic, and repair all render `@2`, preserve the deterministic percentile rank, and use the same pair-identity mechanical gate. The exact-render fixture advances to version 2 while retaining the three superseded `@1` hash triples. Focused Prompt, Schema, preparation, Term Pool, golden, and checkpoint gates reached 53/53; the fresh Runner suite reached 284/284 and build passed. The active Prompt-set fingerprint is `18c5a88e6292b996346a940b55e143c9858a407a568314e5842e4d68853eb1c4`. Runner push remains blocked because no remote is configured.
