...
Implementation acceptance for Q503 is recorded at Runner commits `91e4658`, `ad6f055`, and `da024f0`. `attribute-proposal@2`, `attribute-critic@2`, and `attribute-repair@2` consume one authenticated Snapshot-derived projection that binds the producer ID, Snapshot, dispositions, complete Term bindings, and ordered projected output; exact base/repair rendering and checkpoint lookup/write use the same projection identity. A persistence fixture proves zero-call reuse after the first projection-bound checkpoint write. Independent review rejected incomplete authority and a write-path identity shadow before returning final `READY` after both corrections. The fresh full suite is 285/285 and build passes. The active Prompt-set fingerprint is `18c5a88e6292b996346a940b55e143c9858a407a568314e5842e4d68853eb1c4`. Proposal contract hash is `62ad4153533910b5d071ac60e10532afcdd7e2cfe382d1ad8c3e61106e640c9e`, Critic contract hash is `5b423d88cfe6d56d54a1640a8b0b576ca0bd6e1b42ec7e94f29e9987651395f2`, and repair contract hash is `3b19e086aff37153b4a79d0582337d1dc24a585f10fafe9994f6ef1715b7488b`. The prior `@1` render hashes remain retained in the Runner golden as superseded audit values. This acceptance adds no Q-number and does not modify Q001-Q503.

### Q504: Generic Filter Discard Repair Calls

- Q504: Q183/Q196/Q408 repair is one closed Runner-owned chain and never a caller callback. It starts only when an authorized Generic Filter process discard, not a semantic `drop`, leaves a `linked` Source with no usable Term. Affected Sources are processed once in Snapshot order after the ordinary filter decisions complete. For each Source, the Runner first sends one atomic `term-extraction@2` request with the exact ordinary single-Source dynamic-input shape. Structural exhaustion reaches Q399's stage terminal; it is never treated as an empty extraction. Literal materialization removes nonliteral and normalized-duplicate strings and rejects any candidate whose normalized text was already decided `drop` or process-discarded in the current Generic Filter stage. Remaining new candidates are sent through `generic-filter@1` in their extraction order; only `keep` candidates become repaired Terms. An atomic recovery-candidate structural failure consumes the same original 2%/20 Generic Filter process-discard budget and audit namespace, and does not recursively enter this chain.
- If at least one repaired Term remains, the Source stays `linked`; all repaired Terms are appended after the original retained Terms by affected-Source order then extraction order, and `sourceTermIndex` follows that same canonical Term order. If none remains, the exact ordinary Source Disposition contracts run over the one narrow Source: classify, an always-fresh independent review, and anonymous adjudication only on disagreement. A final legal `excluded` outcome performs the Q184 `filterDiscard` audit repair. A final `linked` outcome is the sanitized terminal `filter_discard_repair_unresolved`; lack of an obvious Term is never converted into `fragment`, `boilerplate`, or another guessed reason. Structural exhaustion in any Disposition call reaches Q418's shared terminal.
- This chain reuses byte-identical `term-extraction@2`, `generic-filter@1`, and Source Disposition `@1` contracts, schemas, examples, repair Prompt, and Critic independence; no Prompt contract version or Prompt-set fingerprint changes. The Runner-owned chain identity is `generic-filter-discard-repair@1`. Its checkpoint binds the Snapshot, pre-filter Term/index/Disposition inputs, ordinary keep/drop/process-discard results, exact ordered repair outputs, and all actual executor traces/provider activity. A chain identity/input change invalidates Generic Filter output, finalized Term Pool, Attributes, Evidence Clusters, all Runs, Revisions, scoring, selection, and publication, but not the original Term Extraction checkpoint or the pre-filter shared Source Disposition checkpoint. Fixtures must cover semantic-drop non-trigger, exact one-time Source order, repeated/dropped/nonliteral recovery candidates, successful multi-Term relink, shared-budget exhaustion, extraction and Disposition structural terminals, independent review/adjudication, explicit exclusion audit, unresolved-linked terminal, actual ledger accounting, checkpoint reuse/invalidation, and zero real/image/SenseNova/art/2.5D calls.

Implementation acceptance for Q504 is recorded at Runner commits `123aebe`, `036901b`, `cf61dfd`, `849828e`, `78cda8e`, `ab43757`, `a5c0c50`, and `94e8b9e`. The caller repair callback is absent; the Runner-owned chain uses the exact retained Prompt contracts through the authenticated executor and ledger boundary, persists only canonical results and sanitized observations, rejects checksum-consistent semantic cache drift, and reuses a matching chain checkpoint with zero provider calls. Recovery drop/process-discard decisions remain unavailable across all later affected Sources in the same stage. The fresh full suite is 298/298 and build passes. No Prompt contract, contract hash, golden render, or Prompt-set fingerprint changes. The constrained reviewer was mechanically unavailable because its local calibration gate is disabled; no independent acceptance verdict is asserted. This acceptance adds no Q-number and does not modify Q001-Q504.

The next decision identifier is **Q505**.
