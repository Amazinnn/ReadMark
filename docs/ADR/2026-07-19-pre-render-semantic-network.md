...
### Q502: Deterministic Filtered Term Ranking And Pool Finalization

- Q502: After deterministic literal validation and Generic Filter, `term-pool-finalizer@1` assigns the only legal Revision `percentileRank` values and applies Q249's 800-Term ceiling. It receives the corrected Source Snapshot, accepted Source Dispositions, retained literal Terms with their complete `sourceTermIndex`, and no model output beyond the already accepted keep/drop decisions. Excluded Sources never contribute. Every retained Term binding is recomputed as the complete Snapshot-ordered Linked Source set containing the exact literal under Q250 matching; a mismatch between that set, the supplied Term binding, and `sourceTermIndex` is `term_pool_input_invalid`, never repaired by guessing.
- Each Term receives five deterministic `0..10000` positive signals. `frequency = roundHalfUp(10000 * boundSourceCount / maximumBoundSourceCount)`. `distribution = roundHalfUp(5000 * distinctTopLevelHeadingBuckets / allLinkedTopLevelHeadingBuckets) + roundHalfUp(5000 * distinctOccupiedPositionDeciles / allLinkedOccupiedPositionDeciles)`, using Q499's root bucket and midpoint deciles. `heading = roundHalfUp(10000 * boundSourcesWhoseHeadingPathContainsTheQ250NormalizedTerm / boundSourceCount)`. `readerSignal = roundHalfUp(10000 * boundSourcesWhereAReaderMarkerOverlapsALiteralOccurrence / boundSourceCount)`; only `reader_comment`, `highlight`, and `bold` count, while `callout` is structural context and contributes zero. `technical = 10000` only when the literal contains a Latin letter and also contains an ASCII digit, an embedded `+`, `#`, `.`, `/`, `:`, `_`, or `-`, an internal lowercase-to-uppercase transition, or at least two uppercase Latin letters; otherwise it is zero. Marker overlap is computed from exact UTF-16 ranges and a literal occurrence in the Source text; table normalized text has no raw-coordinate marker inference.
- The positive base is `25*frequency + 25*distribution + 20*heading + 15*readerSignal + 10*technical`. Near-duplicate comparison uses Q250-normalized Unicode code-point bigram Jaccard, with a one-code-point normalized string represented by its singleton. A pair is near-duplicate only at Jaccard at least `0.80`. The loser is the lower positive base, then the longer normalized code-point length, then the greater canonical `termId`; it receives `duplicatePenalty = roundHalfUp(2000 * jaccardNumerator / jaccardDenominator)`, taking the maximum across pairs, while the winner receives no penalty from that pair. `globalScore = positiveBase - 100*duplicatePenalty`. Global order is decreasing `globalScore`, decreasing `frequency`, earlier bound Source position, then `termId`. All arithmetic is integer and uses Q499's `roundHalfUp`; floating-point comparison is forbidden.
- If at most 800 Terms remain, final order is global order. Otherwise the first 600 are the global top 600. The remaining Terms are queued into every combined `(topLevelHeadingBucket, positionDecile)` occupied by a binding, each queue in global order; buckets are ordered by their earliest Snapshot Source position, then heading string, then decile. One not-yet-selected Term is taken from each bucket per round, skipping duplicates, until 200 distribution slots are filled or every queue is exhausted. Any unused slots are filled from remaining global order. The final pool is exactly `min(800, retainedTermCount)` Terms; `sourceTermIndex` is pruned to those IDs without reordering.
- Within the final pool, equal `globalScore` values share the average one-based ordinal rank they occupy in global order. Let `rank2 = firstOccupiedOrdinal + lastOccupiedOrdinal`; `percentileRank` is `10000` for a one-Term pool and otherwise `roundHalfUp(10000 * (2*poolSize - rank2) / (2*(poolSize - 1)))`, so all operands remain integers, the highest non-tied score is `10000`, and the lowest is `0`. The emitted Revision `TermBinding.sourceIds` is the complete Linked binding set, not Q151's at-most-ten representative usage sample. Before Attribute Proposal, the pool must contain at least `max(30, ceil(0.80 * targetConceptCount))` Terms; otherwise finalization stops with `term_pool_below_concept_minimum` and never pads. The finalizer checkpoint binds `term-pool-finalizer@1`, Snapshot/Disposition/retained-Term/index hashes, ordered output, and its integer signal version. It invalidates Attributes, Evidence Clusters, all Runs, Revisions, scoring, and selection, but not extraction, Generic Filter, or Source Disposition. Fixtures must cover every signal, callout exclusion, table-coordinate boundary, duplicate winner/penalty, integer ties, 600+200 selection, multi-bucket de-duplication, percentile ties/endpoints, minimum failure, and repeatability. Q138-Q144, Q249-Q251, and Q323 remain retained; implementation-time arbitrary `percentileRank` values are superseded.

### Q503: Attribute Distribution Projection

- Q503: The deterministic Term Pool additionally projects at most twelve canonical occupied `(topLevelHeadingBucket, positionDecile)` pairs for every finalized Term at the Attribute Prompt boundary exactly as frozen in Prompt ADR Q503. The projection uses no new score, semantic inference, caller-selected sampling, or free-form label; it is recomputed from the Term's complete Linked Source bindings and Q499/Q502 bucket definitions by sorting unique occupied pairs by earliest Snapshot Source position, heading string, and decile, then retaining the first twelve. It is Runner-intermediate Prompt input only and does not add fields to final `TermBinding` or any protocol-v3 published artifact. Its canonical ordered output and version participate in the Attribute-stage dependency fingerprint; changes invalidate Attributes and descendants but not the accepted Term Pool artifact itself.

### Q504: Generic Filter Discard Repair Ownership

- Q504: The Runner owns `generic-filter-discard-repair@1` exactly as frozen in Prompt ADR Q504. The chain replaces the implementation-time `repairLinkedSource` callback; callers cannot inject replacement Terms or exclusion reasons. It reuses the accepted Term Extraction, Generic Filter, and Source Disposition contracts through their normal executor/checkpoint/ledger boundary, accepts only new literal candidates that survive Generic Filter, and accepts an exclusion only from the full classify/review/adjudicate outcome. A final `linked` Source without a usable Term fails `filter_discard_repair_unresolved`; no deterministic or model-independent default disposition exists. The chain changes no published protocol shape and no Prompt identity, but its own dependency identity invalidates Generic Filter descendants.

### Q505: Post-Disposition Concept Target Ownership

- Q505: Implementation mechanically proves that Q174 cannot coexist with the current pre-Runner Source Snapshot wire: Q174 derives `targetConceptCount` from final `linkedSourceCount`, while the current wire requires the Plugin to write that value into the immutable, hashed Source Snapshot before Source Disposition exists. Q174 and its original text remain retained; only the impossible pre-Disposition placement is superseded. `targetConceptCount` is removed from Source Snapshot and is never supplied by the Plugin, task bundle, provider, fixture caller, or checkpoint default. After the complete final shared Source Disposition, the Runner computes exactly `clamp(round(4 * sqrt(linkedSourceCount)), 30, 160)` once and passes that deterministic scalar to Term Pool finalization, Attribute acceptance, Evidence Cluster construction, all three Runs, Revision validation, and scoring. Published Revisions already contain the complete Source Disposition, so Runner and Plugin validators and scorers independently recompute the same target from that closed list rather than storing a second target field. Zero Linked Sources fails before target computation; no placeholder, total-Source approximation, migration inference, or compatibility default is legal.
- Removing the impossible field advances the closed Schema bundle and manifest identity while retaining `protocolVersion:3`; every Snapshot, task bundle, checkpoint, Revision, Selection Report, and completion marker bound to the superseded manifest becomes stale and is rejected, not migrated. Prompt-visible contracts change only where `targetConceptCount` already appears as an explicit post-Disposition stage input; Source-only stages do not receive it. Fixtures must prove old Snapshot rejection, exact formula endpoints, post-Disposition ownership, Term Pool/Attribute/Cluster propagation, independent Runner/Plugin recomputation, selective checkpoint invalidation, byte-identical cross-repository Schema bundles, and zero provider/image/SenseNova/art/2.5D calls during target computation.
